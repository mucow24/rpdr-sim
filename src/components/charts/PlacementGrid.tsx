  ...
  useEffect(() => {
    ...
    for (const queen of sortedQueens) {
      ...
      for (const place of placesRightToLeft) {
        ...
        const brightness = normalized * normalized;
        const cx = x(place) ?? 0;
        const cy = y(queen.id) ?? 0;

        // Dark background so zero-probability cells are visible
        g.append('rect')
          .attr('x', cx)
          .attr('y', cy)
        ...
        // Queen-colored overlay, opacity = probability
        g.append('rect')
          .attr('x', cx)
          .attr('y', cy)
          .attr('width', x.bandwidth())
          ...
          .attr('opacity', isFaded ? brightness * 0.3 : brightness)
          .attr('stroke', 'none')
          .style('pointer-events', 'none');

        // Invisible hit rect on top for tooltip/click
        g.append('rect')
          .attr('x', cx)
          .attr('y', cy)
          .attr('width', x.bandwidth())
